...
var writeLib = function(){
		...
		/*song_album_module css*/     $('head').append('<link rel="stylesheet" type="text/css" href="'+$context_root+'resources/song_album_module/css/song_album_module.css">');
...
$(document).ready(function() {
	...
	var d_counter = function(){
		$('#rec_counter').html("");
		drawCounter(cnt_root, user_id);
			...
			$('#avg_star_point_cnt').text(data[0].cnt + "명이 참여했어요.");
				...
				$('#myModal').on('hidden.bs.modal',function(){
					...
					var makeHtml = function(index, item) {
						...
						console.log(item.album_name + " : " + item.album_name.length);
